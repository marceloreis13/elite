				...
				if(('ontouchstart' in window) || window.DocumentTouch && document instanceof DocumentTouch){
					...
					**/
					if(options.typeEvent == 'click'){
						 ...
						 elem.on(clickEvent, this,function(){
							...
							**/
							if(options.effect == 'fade'){
								...
								**/								
								if($(this).next('div:last-child').is(':hidden')){

									/**
									* Hide all open menus if a user clicks on a new button
									* this will prevent multiple open menu's.
									**/
									obj.children('li').children('div:last-child').fadeOut(options.speed);
																	
									$(this).next('div:last-child').stop(true, true).fadeIn(options.speed);
								}else{
									$(this).next('div:last-child').stop(true, true).fadeOut(options.speed);	
								}
							}else{
								/**
								* Hide all open menus.
								**/
								obj.children('li').not($(this).parent()).children('div:last-child').hide();	
															
								/**	
								* Animate menu(toggle height).
								**/
								$(this).next('div:last-child').stop(true, true).animate({height: 'toggle'},options.speed);								
							}
						...
						**/							 
						elem.on("mouseenter tap", function(){	
							$(this).addClass(options.activeClass);	
						...
						**/												
						elem.parent('li').on("mouseleave tap", function(){	
